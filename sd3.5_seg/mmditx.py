...
def block_mixing(context, x, context_block, x_block, c):
    ...
    if not context_block.pre_only:
        ...
        print('context',context.shape)#Size([2, 154, 1536])

    else:
        context = None
        print('context noneeeeeeee' )
 

    if x_block.x_block_self_attn:
        x_q2, x_k2, x_v2 = x_qkv2
        attn2 = attention(x_q2, x_k2, x_v2, x_block.attn2.num_heads)
        #print('attn2--',attn2.shape)  ##([2, 4096, 1536])
        x = x_block.post_attention_x(x_attn, attn2, *x_intermediates)
        #print('x90',x.shape) ##([2, 4096, 1536])
    else:
        x = x_block.post_attention(x_attn, *x_intermediates)
        #print('x-0-0',x.shape)  ##([2, 4096, 1536])
    print('block_mixing--ee')
...
class MMDiTX(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        #print('--------------------')
        #print('input_size',input_size) #None
        #print('in_channels',in_channels) #16
        #print('hidden_size',hidden_size) #1536  hidden_size = 64 * depth
        #print(sss)
    ...
    def cropped_pos_embed(self, hw):
        ...
        print('self.pos_embed_max_size',self.pos_embed_max_size)
        ...
        #print('spatial_pos_embed',spatial_pos_embed.shape)  #([1, 4096, 1536])
    ...
    def forward_core_with_concat(
        ...
        for i, block in enumerate(self.joint_blocks):
            if i in skip_layers:
                continue
            #print('c_mod',c_mod.shape)
            context, x = block(context, x, c=c_mod)
            print('x0909123',x.shape)
            
            dx=x.clone()
            ddx = self.final_layer(dx, c_mod)
            outputatt.append(ddx)
            #print('x0909123',x.shape)
        ...
        #print('x-0-0-11',x.shape)  ##[2, 4096, 64])
        return x, outputatt
    ...
    def forward(
        ...
        hw = x.shape[-2:]  
        #print('x',x.shape)
        #print('hw',hw)  ##torch.Size([128, 128])
        x = self.x_embedder(x) + self.cropped_pos_embed(hw)
        #print('x',x.shape) #([2, 4096, 1536])
        #print('t',t)
       # print('t',t.shape)
        ...
        #print('context',context.shape) #([2, 154, 4096])
        context = self.context_embedder(context)
        #print('context',context.shape) #([2, 154, 1536])

        x , outputatt= self.forward_core_with_concat(x, c, context, skip_layers, controlnet_hidden_states) #([2, 4096, 64])
        #print('x',x.shape) #([2, 4096, 64])
        att_sss1 =[]
        for i, att_sss in enumerate(outputatt):
             att = self.unpatchify(att_sss, hw=hw)
             att_sss1.append(att)
        x = self.unpatchify(x, hw=hw)  # (N, out_channels, H, W) #([2, 16, 128, 128])
        #print('x22',x.shape)  
        return x, att_sss1
