...
class SD3Inferencer:
    ...
    def do_sampling(
        ...
        # print('denoise=1.0,',denoise)
        # print('skip_layer_config',skip_layer_config)
        ...
        # for i, att in enumerate(denoised_ss):
            ...
            # img_out = np.array(img_out, dtype=np.uint8)
            # img_out = cv2.applyColorMap(img_out,cv2.COLORMAP_JET)
            # save_namecc = f"img50/attdis_{i+1}.jpg"
            # cv2.imwrite(save_namecc,img_out) 
        
        # for i, att in enumerate(outputatt_sss):
            ...
            # img_out = np.array(img_out, dtype=np.uint8)
            # img_out = cv2.applyColorMap(img_out,cv2.COLORMAP_JET)
            # save_namecc = f"img24/attdis_{i+1}.jpg"
            # cv2.imwrite(save_namecc,img_out)         
            
            
        # for i, att_sss in enumerate(outputatt_sss):
           # for j, att in enumerate(att_sss):
            # img_out = att[0,:,:,:].clone()
            # img_out = img_out.mean(axis=0).cpu().numpy()
            # #img_out=F.interpolate(img_out, (h, w), mode="bicubic")
            # cv2.normalize(img_out, img_out, 0, 255, cv2.NORM_MINMAX)
            # img_out = np.array(img_out, dtype=np.uint8)
            # img_out = cv2.applyColorMap(img_out,cv2.COLORMAP_JET)
            # save_namecc = f"img/attdis_{i+1}_{j+1}.jpg"
            # cv2.imwrite(save_namecc,img_out)           
            
            
            
        #print('outputatt_sss',outputatt_sss[20].shape)
        #print('denoised_ss',denoised_ss[4].shape)
        #print('denoised_ss',denoised_ss[5].shape)
