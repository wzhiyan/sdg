...
class BaseModel(torch.nn.Module):
    ...
    def apply_model(self, x, sigma, c_crossattn=None, y=None, skip_layers=[], controlnet_cond=None):
        ...
        model_output, outputatt = self.diffusion_model(
            x.to(dtype),
            timestep,
        ...
        return self.model_sampling.calculate_denoised(sigma, model_output, x), outputatt
...
class CFGDenoiser(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #print('uncond',uncond)
        batched,outputatt = self.model.apply_model(
            torch.cat([x, x]),
        ...
        for i, att_sss in enumerate(outputatt):
            ...
            att_scaled.append(scaled)       
 
        
        return scaled,att_scaled
...
def sample_dpmpp_2m(model, x, sigmas, extra_args=None):
    ...
    for i in tqdm(range(len(sigmas) - 1)):
        ...
        denoised,outputatt = model(x, sigmas[i] * s_in, **extra_args)
        #outputatt_sss.append(outputatt)
        denoised_ss.append(denoised)
