...
import os
from other_impls import Mlp, attention,attention_ks
import cv2
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x, out_per_head = attention_ks(q, k, v, self.num_heads)
...
def block_mixing(context, x, context_block, x_block, c):
    ...
    print('context_qkv[0].shape[1]',context_qkv[0].shape[1])
    q, k, v = tuple(
        torch.cat(tuple(qkv[i] for qkv in [context_qkv, x_qkv]), dim=1)
        for i in range(3)
    )
    attn,out_per_head = attention_ks(q, k, v, x_block.attn.num_heads)
    #print('attn',attn.shape)
    #print('out_per_head',out_per_head.shape)
 

    x_out_per_head =  out_per_head[:,:, context_qkv[0].shape[1] :]
    ...
    #print('attn',attn.shape)
    #print('context_attn',context_attn.shape)
    #####print(sss)
    img_out_z = x_attn.clone()
    img_out_z =img_out_z.view(2,1536,4096)
    img_out_z =  img_out_z.unfold(2,64,64)

    img_out_z =img_out_z[0,:,:,:]
    img_out_z = img_out_z.mean(axis=0).cpu().numpy()
    img_out_z =img_out_z.astype(np.float32)
    cv2.normalize(img_out_z, img_out_z, 0, 255, cv2.NORM_MINMAX)
    img_out_z = np.array(img_out_z, dtype=np.uint8)
    img_out_z = cv2.applyColorMap(img_out_z,cv2.COLORMAP_JET)
    cv2.imwrite('sasa.jpg',img_out_z)   
    
    img_out__ =x_out_per_head[0,:,:,:].clone()
    for i in range(img_out__.shape[0]):
        headatt_ = img_out__[i]
        headatt = headatt_.permute(1,0)
        headatt = headatt.unfold(1,64,64)
        
        img_out = headatt.mean(axis=0).cpu().numpy()
        ...
        save_namecc = f"att/attdis_{i+1}.jpg"
        cv2.imwrite(save_namecc,img_out)   
...
class MMDiTX(nn.Module):
    ...
    def forward(
       ...
       #print('img_out--',img_out)
        ...
        img_out = np.array(img_out, dtype=np.uint8)
        img_out = cv2.applyColorMap(img_out,cv2.COLORMAP_JET)
        cv2.imwrite('sfinal.jpg',img_out)  
        return x
