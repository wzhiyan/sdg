...
from other_impls import Mlp, attention,attention_kc
...
def block_mixing(step,di,context, x, context_block, x_block, c):
    print('0-0-',step)
    ...
    if step < 1 or (15 < step < 17) or (32 < step < 34) or (46 < step < 50):
        attn = attention_kc(step,di,q, k, v, x_block.attn.num_heads)
    else: 
...
class MMDiTX(nn.Module):
    ...
    def forward_core_with_concat(
        ...
        for i, block in enumerate(self.joint_blocks):
            if i in skip_layers:
                continue
            context, x = block(itit,i,context, x, c=c_mod)
    ...
    def forward(
        ...
        x = self.forward_core_with_concat(i,x, c, context, skip_layers, controlnet_hidden_states)
