...
def attention_ks(q, k, v, heads, mask=None):
    """Convenience wrapper around a basic attention operation"""
    b, _, dim_head = q.shape
    dim_head //= heads
    q, k, v = map(lambda t: t.view(b, -1, heads, dim_head).transpose(1, 2), (q, k, v))
    out = scaled_dot_product_attention_kc(
        q, k, v, attn_mask=mask, dropout_p=0.0, is_causal=False
    )
    #print('out',out.shape)
    out_per_head = out
    return out.transpose(1, 2).reshape(b, -1, heads * dim_head),out_per_head

def attention_kc(itttit,di,q, k, v, heads, mask=None):
    """Convenience wrapper around a basic attention operation"""
    b, _, dim_head = q.shape
    dim_head //= heads
    q, k, v = map(lambda t: t.view(b, -1, heads, dim_head).transpose(1, 2), (q, k, v))
    out = scaled_dot_product_attention_kc(
        itttit,di,q, k, v, attn_mask=mask, dropout_p=0.0, is_causal=False
    )
    return out.transpose(1, 2).reshape(b, -1, heads * dim_head) 
...
def scaled_dot_product_attention_kc(itttit,di,query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None) -> torch.Tensor:
        ...
        context_attn_weight = attn_weight[:, : ,:154,:154].clone()
        context_attn_weight_sum = torch.sum(context_attn_weight,dim=1).cpu().numpy()
        print('context_attn_weight_sum',context_attn_weight_sum.shape)        
        
        context_attn_weight = context_attn_weight.cpu().numpy()
        print('context_attn_weight',context_attn_weight.shape)
        #print('context_attn_weight',context_attn_weight)
        path=f"./figures_z/step_{itttit}/d_{di}"
        for ii in range(context_attn_weight.shape[1]):
            print(ii)
            
            attention_plot(context_attn_weight[0, ii, :, :], annot=True, x_texts=None, y_texts=None, figsize=(154, 154), figure_path=path,
                           figure_name='attention_weight_head_{}.png'.format(ii+1))

        attention_plot(context_attn_weight_sum[0, :, :], annot=True, x_texts=None, y_texts=None, figsize=(154, 154), figure_path=path,
                           figure_name='attention_weight_head_sum.png')
