...
def main(args):
    ...
    logging.basicConfig(filename="logfile_ip_ic.log", level=logging.INFO)
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if "_gtFine_color.png" in file:
                ...
                image_path_gt = image_path_label.replace(
                    "gtFine_color.png", "gtFine_labelIds.png"
                )
                ...
                image_ip = image_path_label.replace(
                ...
                outputs = pipe_i2t(ip_adapter_image)
                ...
                save_gen = save_gen.replace(
                    "gtFine_trainvaltest/gtFine/train", "syc_ic"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process SD based semantic controlnet on cityscapes dataset"
    )
    ...
    # add an argument with --ip_adapter_image with default false, only enable when the argument is passed
    parser.add_argument(
        "--ip_adapter_image", action="store_true", help="Enable ip_adapter_image"
    )
